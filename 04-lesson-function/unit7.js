...
// Результат (значение переменной a2) будет сохранен в переменной result.
let result = t2(a2);

// Теперь результат можно использовать для вывода в любой нужный блок, в том числе и в .out-2.
let out2 = document.querySelector('.out-2');
out2.textContent = result;


const newLocal = document.querySelector('.b-2').onclick = function () {
...
function t5(yourName) {
    let res = yourName.length;
    return res;
}

document.querySelector('.b-5').onclick = function () {
    document.querySelector('.out-5').textContent = t5('Diana');
}
...
function t6(a, b) {
    ...
    let randomNumber = Math.floor(randomFraction * (b - a + 1)) + a;
    return randomNumber; 
} 
...
function t7() {
    ...
    // Формируем строку в формате "rgb(x, y, z)"
    let color = `rgb(${x}, ${y}, ${z})`;

    // Возвращаем сгенерированный цвет
    return color;
}
...
document.querySelector('.b-10').onclick = function () {
    document.querySelector('.out-10').textContent = t10(7, 7);
}